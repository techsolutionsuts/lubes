...
mysql_query("insert into tbproduct(prod_code,cat_id,suplierid,product_name,quantity,Qty_Left,content,price,unit_price,Description,date,Driver_num,image,DueDate,invoicevalue,receiver,Driver_name,car_num,invoice_num,System_Date,Personel,ppd) 
                            values('$productcode','$catid','$suplierid','$product_name','$quantity','$quantity','$content','$price','$unit_price','$Description','$aridate','$drivernum','$store','$duedate','$pcost','$personrec','$dname','$car','$invoice_num',now(),'$personel','$ppd')") or die(mysql_error('Please the product already Exist so give it a different product code'));

mysql_query("insert into tballproduct(prod_code,cat_id,suplierid,product_name,quantity,Qty_Left,content,price,unit_price,Description,date,Driver_num,image,DueDate,invoicevalue,receiver,Driver_name,car_num,invoice_num,System_Date,Personel,ppd) 
                            values('$productcode','$catid','$suplierid','$product_name','$quantity','$quantity','$content','$price','$unit_price','$Description','$aridate','$drivernum','','$duedate','$pcost','$personrec','$dname','$car','$invoice_num',now(),'$personel','$ppd')") or die(mysql_error());
...
mysql_query("insert into tb_stock_dynamics(productid,productcode,date,open_stock,total_stock,closing_stock,price,invoices,user,system_date,description,receive,ppd) values('$pid','$productcode','$aridate','$quantity','$quantity','$quantity','$price','$invoice_num','$personel',now(),'$suplierid','$personrec','$ppd')") or die(mysql_error());


mysql_query("insert into retail_price (prodid,system_date,effective_date,product,personel,price) values('$pid',now(),'$aridate','$product_name','$personel','$price')") or die(mysql_error());

mysql_query("insert into unit_price (prodid,system_date,effective_date,product,personel,price) values('$pid',now(),'$aridate','$product_name','$personel','$unit_price')") or die(mysql_error());
...
mysql_query("insert into tb_stock_dynamics(productid,productcode,date,receipt_reduce_stock,total_stock,closing_stock,price,invoices,user,system_date,description,receive,ppd) values('$pid','$productcode','$aridate','$quantity','$quantity','$quantity','$price','$invoice_num','$personel',now(),'$suplierid','$personrec','$ppd')") or die(mysql_error());
...
<tr><td>
  ...
  <span style="color: black;">Supplier:</span>
	  ...
	  <tr><td>
     <span style="color: black;">Quantity:</span><input type="text" min="1" id="txtqty" onkeyup="sum();" name="quantity" placeholder="Quantity" required style="width:265px; height:30px;">
     ...
     <span style="color: black;">Product Category:</span>
	  ...
	  <tr><td>
	   ...
	   <select style="width:265px; height:30px; margin-left:-5px;" name="content" >
	  <?php
	  $c = mysql_query("select * from tbcategory");
	  while($row = mysql_fetch_object($c))
	  {
	  ?>
	  <option value ="<?= $row->cat_content ?>"><?= $row->category_name ?><?= $row->cat_content ?></option>
	  <?php
	  ...
	  ?>
	   ...
	   <span style="color: black;">Description:</span><input type="text" placeholder="Product Description" name="Description" required style="width:265px; height:30px;"></td></tr>
     <br><br>
     <tr><td>
    ...
    <tr><td>
      <span style="color: black;">PPD:</span><input type="text" id="txt3" placeholder="PPD Date" name="ppd"  style="width:265px; height:30px;">
     </td></tr>
     <tr><td>
