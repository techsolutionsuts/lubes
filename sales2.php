...
mysql_query("insert into tb_stock_dynamics(productid,productcode,date,closing_stock,qty_sold,price,value,invoices,user,system_date,description,sid) values('$proid','$code','$date','$closing','$qtysold','$pprice','$withdiscot','$invoi','$user',now(),'$discrp','$sid')") or die(mysql_error());


header("location: sales2.php?id=$w&invoice=$invoi");
...
?>
   ...
   <table class = "table">
    ...
    $id=$_GET['invoice'];
          ...
          ?>
    <input type="text" hidden name="aridate" required style="width:300px; height: 35; margin-top: 35px; left:60px; position: absolute;" value="<?php echo $bringd ?>" />
  ...
  <?php
  include('db/connect.php');
  $result = $db->prepare("SELECT * FROM tbproduct where Qty_Left>0");
...
<br><br>
      ...
      <font style="color:rgb(255, 95, 66);; font:bold 18px 'Aleo';"><?php echo $selectprod;?></font> 
      ...
      ?>
                ...
                <td width="90"><a href="delete.php?id=<?php echo $row['salesDid']; ?>&invoice=<?php echo $_GET['invoice']; ?>&dle=<?php echo $_GET['id']; ?>&qty=<?php echo $row['Quantity']; ?>&code=<?php echo $row['Productid'];?>"><button class="btn btn-mini btn-warning"><i class="icon icon-remove"></i> Remove </button></a></td>
                </tr>
                <?php
                  }
