...
if(isset($_GET['from_sales_date']) && isset($_GET['to_sales_date']) && $_GET['from_sales_date']!='' && $_GET['to_sales_date']!='' && $_GET['towhom']!='0')
{

  $frdate = $_GET['from_sales_date'];
  $tdate = $_GET['to_sales_date'];
  $pr = $_GET['towhom'];
  $save = $_GET['save'];

  if ($_GET['towhom'] == 'Dormaa Shell') {
  header("location: transfrmDm.php?fr=$frdate&to=$tdate&tw=$pr&save=$save");
  }
else{
...
$else =("$person between ".$disfdate. " to ".$distdate);
...
<center><font color="green" style="font:bold 15px 'Aleo';"><?php echo $else;?> </font></center>              
	  ...
	  <?php
	  $q =mysql_query("select * from subtracted_products where why = '$person' AND date BETWEEN '$fromdate' AND '$todate' order by date");
