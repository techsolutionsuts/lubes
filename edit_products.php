...
$result = $db->prepare("SELECT * FROM supliers WHERE suplierid= :userid");
$result->bindParam(':userid', $suplierid);
$result->execute();
...
mysql_query("update tbproduct set   product_name  = '$product_name',
									...
									ppd           =  '$ppd' where id = '$id'") or die(mysql_error()); 
...
mysql_query("insert into tballproduct(prod_code,cat_id,suplierid,product_name,quantity,Qty_Left,content,price,unit_price,Description,date,Driver_num,image,DueDate,invoicevalue,receiver,Driver_name,car_num,invoice_num,System_Date,Personel,ppd) 
                            values('$productcode','$catid','$supplier','$product_name','$quantity','$qtyleft','$content','$price','$unit_price','$Description','$aridate','$drivernum','','$duedate','$pcost','$personrec','$dname','$car','$invoice_num',now(),'$personel','$ppd')") or die(mysql_error());

mysql_query("insert into tb_stock_dynamics(productid,productcode,date,receipt_reduce_stock,total_stock,closing_stock,invoices,user,system_date,description,receive,ppd) values('$id','$product_code','$aridate','$quantity','$qtyleft','$qtyleft','$invoice_num','$personel',now(),'$supplier','$personrec','$ppd')") or die(mysql_error());     
...
<?php 
    ...
    <span style="color: black;">Price</span><input type="text" readonly placeholder="Price" name="price" value="<?= $row->price ?>" required style="width:265px; height:30px;">
     ...
     <input hidden value="<?= $row->cat_id ?>" name="catid" style="width:265px; height:30px; margin-left:-5px;">
      ...
      <tr><td>
      	    <span style="color: black;">PPD:</span><input type="text"  placeholder="PPD Date" name="ppd" required style="width:265px; height:30px;">
