...
$fromdate=$mysqldate;
$sal =("Products Received on ".$disfdate);
...
<center> <!--<strong>Date : <input type="date" style="width: 223px; height: 20px; padding:14px;" name="from_sales_date" required ></strong>-->
  ...
  $result = $db->prepare("SELECT date FROM tballproduct  group by date");
    $result->bindParam(':userid', $res);
    $result->execute();
    for($i=0; $row = $result->fetch(); $i++){
  ?>
    <option value="<?php echo $row['date'];?>"><?php  $mysqldate=$row['date']; $phpdate = strtotime( $mysqldate ); $phpdate = date("d/m/Y",$phpdate);
    echo $phpdate; ?></option>
  <?php
        }
      ?>
...
<?php echo $sal; ?><br>
    ...
    <th>PPD</th>
	  ...
	  $result = $db->prepare("SELECT * FROM tballproduct WHERE date= :userid");
...
<tr>
        ...
        <?php
	          ...
	          include('db/connect.php');
        $result = $db->prepare("SELECT sum(invoicevalue) FROM tballproduct WHERE date= :invoi");
