...
Current Stock Available, Date Time: <?php echo date('d-m-Y h:i:s'); ?><br>
    ...
    <td><?=$row->suplierid ?></td>
	  ...
	  <tr>
        ...
        <?php
	          include('db/connect.php');
	          $less = 0;
        $result = $db->prepare("SELECT sum(unit_price*Qty_Left) FROM tbproduct WHERE Qty_Left > :invoi");
        $result->bindParam(':invoi', $less);
        $result->execute();
        for($i=0; $rows = $result->fetch(); $i++){
        $upr=$rows['sum(unit_price*Qty_Left)'];
        ...
        <?php
	          include('db/connect.php');
        $result = $db->prepare("SELECT sum(price*Qty_Left) FROM tbproduct WHERE Qty_Left > :invoi");
        $result->bindParam(':invoi', $less);
        $result->execute();
        for($i=0; $rows = $result->fetch(); $i++){
        $pp=$rows['sum(price*Qty_Left)'];
        echo formatMoney($pp, true);
        }
        ?>
        </strong></td>
        <tr>
                  <th colspan="5"><strong style="font-size: 12px; color: #222222;">Total Turn Out:</strong></th>
                  <td colspan="1"><strong style="font-size: 12px; color: #222222;"></strong></td>
        <td colspan="1"><strong style="font-size: 12px; color: #222222;">
        <?php
	          $ttout = $pp - $upr;
  ...
  <tr>
                  <th colspan="5"><strong style="font-size: 12px; color: #222222;">Total Literst:</strong></th>
                  <td colspan="1"><strong style="font-size: 12px; color: #222222;"></strong></td>
        <td colspan="1"><strong style="font-size: 12px; color: #222222;">
        <?php
	          include('db/connect.php');
        $result = $db->prepare("SELECT sum(content*Qty_Left) FROM tbproduct WHERE Qty_Left > :invoi");
        $result->bindParam(':invoi', $less);
        $result->execute();
        for($i=0; $rows = $result->fetch(); $i++){
        $pp=$rows['sum(content*Qty_Left)'];
        echo formatMoney($pp, true);
        }
        ?>
        </strong></td>
