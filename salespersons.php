...
$todate=$mysqldate;
$sal =("Individuals sales from ".$disfdate. " To ".$distdate);
...
<center><h4><i class="icon-plus-sign icon-large"></i> Sales Person's Report With a specific Time Interval</h4></center>
...
<ul class="breadcrumb" style="float: left; width: 845; background:   #7FFFD4 ;">
      ...
      <li style="float: left; "><a href="customerreport.php">Customer</a></li>  
			<li class="active" style="float: left;">Sales by Individuals</li>
			</ul>
      <br>
</form>
...
<table border="1" class="table table-bordered" id="resultTable" data-responsive="table">
        ...
        $result = $db->query("SELECT salesperson, count(salesperson) as sp, sum(Liters) as li, sum(Discount) as disct, sum(Profit) as prf, sum(TotalCash) as am FROM sales where salesDate BETWEEN '$fromdate' AND '$todate' GROUP BY salesperson asc");
while ($line = $db->fetchNextObject($result)) {
?>
      
        <tr>
                <td><?php echo $line->salesperson; ?></td>
                <td><?php echo $line->sp; ?></td>
...
<tr>
  ...
  <th colspan="1"><strong style="font-size: 12px; color: #222222;">Totals:</strong></th>

  <td colspan="1"><strong style="font-size: 12px; color: #222222;">
<?php 
         $result = $db->query("SELECT count(salesperson) as sm FROM sales where salesDate BETWEEN '$fromdate' AND '$todate' ");
while ($line = $db->fetchNextObject($result)) {
$sum =$line->sm ;
echo $sum;
...
<td colspan="1"><strong style="font-size: 12px; color: #222222;">
        <?php 
        $result = $db->query("SELECT sum(Liters) as dis FROM sales where salesDate BETWEEN '$fromdate' AND '$todate' ");
while ($line = $db->fetchNextObject($result)) {
$dis = $line->dis;
...
echo formatMoney($li,true);
        ...
        <td colspan="1"><strong style="font-size: 12px; color: #222222;">
<?php 
        $result = $db->query("SELECT sum(profit) as pr FROM sales where salesDate BETWEEN '$fromdate' AND '$todate' ");
while ($line = $db->fetchNextObject($result)) {
$pro =$line->pr ;
...
<td colspan="1"><strong style="font-size: 12px; color: #222222;">
<?php 
         $result = $db->query("SELECT sum(TotalCash) as sm FROM sales where salesDate BETWEEN '$fromdate' AND '$todate' ");
while ($line = $db->fetchNextObject($result)) {
$sum =$line->sm ;
